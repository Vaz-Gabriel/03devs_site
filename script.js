...
(function() {
  ...
  // ============================================
  // RENDERIZAÇÃO 3D (THREE.JS) - CORRIGIDA
  // ============================================
  ...
  function init3D() {
    ...
    // Luzes
    const ambientLight = new THREE.AmbientLight(0xffffff, 1.0); // Luz branca forte
    scene.add(ambientLight);

    const directionalLight = new THREE.DirectionalLight(0x00ffff, 2.0); // Luz ciano direcional
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        const scale = 3.5 / maxDim; // Aumentado um pouco
        ...
        // Aplicar material holográfico suave
        model.traverse((node) => {
          if (node.isMesh) {
  ...
  function animate() {
    ...
    if (model) {
      model.rotation.y += 0.005; // Velocidade reduzida para uma rotação mais suave
    }
